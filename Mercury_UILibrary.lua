--[[
    ...
    Update 09/08 — Compact search + in-window notifications:
      • Search box shrunk back down to a fixed, compact width (120/160px)
        instead of stretching the full toolbar row.
      • Notifications now spawn INSIDE the window, anchored to its own
        bottom-right corner, instead of floating over the whole screen —
...
local Library = {
    ...
    AutoHide      = true,    -- hide the UI after a spell of no interaction
...
-- ─────────────────────────────────────────────────────────────
-- Shared drag dispatcher (perf).
-- Previously every bindDrag() call — and every element that rolled
-- its own drag code (sliders, the colour picker) — opened its OWN
-- UserInputService.InputChanged connection. With N sliders + a title
-- bar + a resize handle + a repeatedly-reopened colour picker, that
-- meant N+ permanent global listeners all evaluating on every single
-- mouse/touch sample on screen, and — for the colour picker — new
-- ones added every time it was opened and never removed.
--
-- There is exactly ONE UserInputService.InputChanged connection for
-- the whole script now. Every active drag just registers itself in
-- activeDrags (keyed by its handle) instead of adding a connection,
-- so cost no longer scales with how many draggable elements exist.
-- ─────────────────────────────────────────────────────────────
local activeDrags = {}   -- [handle] = {startInputPos=Vector2, dragInput=InputObject?, onMove=fn}
...
UserInputService.InputChanged:Connect(function(input)
    ...
    for _, session in next, activeDrags do
        if session.dragInput == input then
            local delta = input.Position - session.startInputPos
            session.onMove(delta, input.Position)
        end
    end
end)
...
local function bindDrag(handle, onMove, onBegin, onEnd)
    handle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1
            ...
            local session = { startInputPos = input.Position, onMove = onMove }
            activeDrags[handle] = session
            sinkCameraInput()                         -- stop camera rotation
            if onBegin then onBegin(session.startInputPos) end

            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    if activeDrags[handle] == session then   -- guard against double-fire
                        activeDrags[handle] = nil
                        releaseCameraInput()
    ...
    handle.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement
            or input.UserInputType == Enum.UserInputType.Touch then
            local session = activeDrags[handle]
            if session then session.dragInput = input end
        end
    end)
end
...
function Library:create(options)
    ...
    --   Lives INSIDE the window now (parented to `core`, not the full
    --   screen) and anchors to the window's own bottom-right corner —
    --   notifications stack up from there instead of floating loose
    --   over the game outside the UI. Inset a little extra at the
    --   bottom so the stack never sits under the resize grip.
    local notificationHolder = core:object("Frame", {
    ...
    -- ----- Search box (row 2) --------------------------------------------
    --   Kept compact and right-aligned rather than stretched full-width —
    --   still sits in its own row below the nav arrows, still filters the
    --   sidebar tabs live as you type.
    local SEARCH_W = IsMobile and 120 or 160
    local searchHolder = titleBar:object("Frame", {
        Theme = { BackgroundColor3 = {"Secondary", 8} },
        BackgroundTransparency = 0.35,
        AnchorPoint = Vector2.new(1, 0),
        Position = UDim2.new(1, -14, 0, TAB_ROW_HEIGHT + 5),
        Size = UDim2.new(0, SEARCH_W, 0, TOOLBAR_ROW_HEIGHT - 10),
        ZIndex = 4
    ...
    do
        ...
        local function stopFollow()
            if dragHeartbeat then
                dragHeartbeat:Disconnect()
                dragHeartbeat = nil
            end
        end
        ...
        bindDrag(titleBar,
            function(delta)
                ...
                -- Just record the target here — no Tween allocation per sample.
                targetPos = UDim2.fromOffset(newX, newY)
            end,
            function()
                startPos, targetPos = core.Position, core.Position
                -- One Heartbeat connection, alive only for this drag, chases the
                -- cursor with the same "feel" DragSpeed used to give via tween
                -- (now an exponential smoothing time-constant instead), without
                -- allocating a new Tween object on every input sample.
                stopFollow()
                dragHeartbeat = RunService.Heartbeat:Connect(function(dt)
                    local speed = math.max(Library.DragSpeed, 0.001)
                    local alpha = 1 - math.exp(-dt / speed)
                    core.Position = core.Position:Lerp(targetPos, alpha)
                end)
            end,
            function()
                stopFollow()
                core.Position = targetPos   -- snap exactly to the release point
            end
        )
    end
...
function Library:slider(options)
    ...
    local function applyValue(value, fireCallback, instant)
        ...
        if instant then
            -- Continuous drag: the thumb should track the finger/cursor exactly,
            -- with no lag — and no per-sample Tween allocation.
    ...
    -- Uses the shared bindDrag dispatcher instead of opening its own
    -- UserInputService connections — with many sliders on one panel this
    -- is the difference between 1 shared listener and 2*N of them.
    do
        local function updateFromPosition(pos)
            ...
            applyValue(value, true, true)   -- instant: no tween while actively dragging
        end

        bindDrag(hitArea,
            function(_, pos) updateFromPosition(pos) end,
            function(pos)    updateFromPosition(pos) end
        )
    end
...
function Library:color_picker(options)
    ...
    row.MouseButton1Click:Connect(function()
        ...
        local function updateColor()
            ...
            -- Direct sets, not tweens: this runs on every drag sample, so a
            -- 0.05s tween here meant a brand-new Tween + Completed connection
            -- allocated on every single mouse-move while dragging.
        ...
        -- Drag SV / Hue — via the shared bindDrag dispatcher. Previously each
        -- one of these opened its own permanent UserInputService.InputChanged
        -- and InputEnded connections, and since the picker can be opened and
        -- closed repeatedly, those connections were never disconnected and
        -- accumulated for the rest of the session (only 1 :Disconnect() call
        -- existed in the whole original file). bindDrag's connections are
        -- shared and scoped to the drag itself, so nothing is left behind
        -- when the picker closes.
        bindDrag(svArea, function(_, pos)
            local rx = math.clamp((pos.X - svArea.AbsolutePosition.X) / svArea.AbsoluteSize.X, 0, 1)
            local ry = math.clamp((pos.Y - svArea.AbsolutePosition.Y) / svArea.AbsoluteSize.Y, 0, 1)
            sat = rx; val = 1 - ry
            updateColor()
        end, function(pos)
            local rx = math.clamp((pos.X - svArea.AbsolutePosition.X) / svArea.AbsoluteSize.X, 0, 1)
        ...
        bindDrag(hueBar, function(_, pos)
            hue = math.clamp((pos.Y - hueBar.AbsolutePosition.Y) / hueBar.AbsoluteSize.Y, 0, 1)
            updateColor()
        end, function(pos)
            hue = math.clamp((pos.Y - hueBar.AbsolutePosition.Y) / hueBar.AbsoluteSize.Y, 0, 1)
            updateColor()
